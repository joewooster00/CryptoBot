...
async def sendMessage(message):
  channel = client.get_channel(900025180758376459)
  await discord.utils.channel.send(message)
#  await discord.utils.get(client.get_channel(900025180758376459)).send(message)
              ...
              if db['hitPriceTarget'] < current_price:
                  if checkTwoListOrder(normal_alert(db['hitPriceTarget'],current_price),db['noti']):
                    for priceTarget in list(set(normal_alert(db["hitPriceTarget"],current_price)) - set(db["noti"])):
                        await channel.sendMessage(f'The price of {crypto} has just passed {priceTarget} USD. The current price is: {current_price} USD.')
                  else:
                    for priceTarget in list(set(normal_alert(db["hitPriceTarget"],current_price)) - set(db["noti"])):
                      await channel.sendMessage(f'The price of {crypto} has just passed {priceTarget} USD. The current price is: {current_price} USD.')
              ...
              # decreasing in value:
                  ...
                  if checkTwoListOrder(reverse_alert(db['hitPriceTarget'],current_price,priceTargets),db["noti"]):
                    for priceTarget in list(set(db["noti"]) - set(reverse_alert(db["hitPriceTarget"],current_price,priceTargets))):
                      await channel.sendMessage(f'The price of {crypto} has just fallen below {priceTarget} USD. The current price is: {current_price} USD.')
                  else:
                    for priceTarget in list(set(db["noti"]) - set(reverse_alert(db["hitPriceTarget"],current_price,priceTargets))):
                      await channel.sendMessage(f'The price of {crypto} has just fallen below {priceTarget} USD. The current price is: {current_price} USD.')
              else:
...
async def on_message(message):
  ...
  if message.content.lower() in db.keys():
    channel = client.get_channel(900025180758376459)
    await channel.send(f'The current price of {message.content} is: {getCryptoPrices(message.content.lower())} USD')

  # list all the available coins
  if message.content.startswith('$list'):
    channel = client.get_channel(900025180758376459)
    cryptoSupportedList = [key for key in db.keys()]
    await channel.send(cryptoSupportedList)

  # check whether a coin is being supported
  if message.content.startswith('$support '):
    channel = client.get_channel(900025180758376459)
    cryptoToBeChecked = message.content.split('$support ',1)[1].lower()
    await channel.send(isCryptoSupported(cryptoToBeChecked))
  ...
  if message.content.startswith('$set '):
    ...
    if isCryptoSupported(cryptoConcerned) and check(priceTargets):
      ...
      await channel.send(f'Successfully set price alert for {db["detect crypto"]} at {list(db["detect price"])} USD.')

    else:
      await channel.send(f'Unsuccessful setting of price alerts. Please try again.')

  if message.content.startswith('$start'):
    channel = client.get_channel(900025180758376459)
    await channel.send(f'Started detecting price alert for {db["detect crypto"]} at {list(db["detect price"])} USD.')
